      ...
      <div class="q-gutter-md">
        <base-section flat bordered :title="$t('guestInformation')">
          <div class="col-xs-12 col-sm-5">
            ...
            <base-input name="first_name" v-model="guestPass.first_name" />
          </div>
          <div class="col-xs-12 col-sm-5">
            <base-label required>{{ $t("surname") }}</base-label>
            <base-input name="last_name" v-model="guestPass.last_name" />
          </div>
          <div class="col-xs-12 col-sm-4">
            <base-label required>{{ $t("emailAddress") }}</base-label>
            <base-input name="email" v-model="guestPass.email" />
          </div>
          <div class="col-xs-12 col-sm-4">
            <base-label required>{{ $t("phoneNumber") }}</base-label>
            <base-input name="phone_number" v-model="guestPass.phone_number" />
          </div>
          <div class="col-xs-12 col-sm-12">
            <base-label>{{ $t("note") }}</base-label>
            <base-input name="note" v-model="guestPass.note" type="textarea" />
          </div>
        </base-section>
      </div>
    </base-form>
