<template>
  <li
    :class="{
      ...
      'q-timeline__entry--icon': hasIcon,
    }"
  >
    ...
    <div class="q-timeline__content">
      <template v-if="creating">
        ...
        <div class="actions-buttons flex items-center">
          <q-space />
          <base-select
            v-if="!noRag"
            class="q-mr-sm"
      ...
      if (this.type === "updated" && this.options) {
        Object.keys(this.options).forEach((key) => {
          ...
          if (key === "password") {
            ...
            attributes.push(
              `${label} changed from <b>${item.previous}</b> to <b>${item.current}</b>`
            );
          }
        });
        message = attributes.join("<br>");
      }
      return this.$core.nl2br(message);
    },
    dateTime() {
