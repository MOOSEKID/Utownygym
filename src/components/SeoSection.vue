<template>
  <base-section :flat="flat" :title="title" :description="description">
    ...
    <template v-if="editable">
      ...
      <div class="col-xs-12 col-sm-12">
        <div class="text-label">Page title</div>
        <q-input
          dense
          outlined
          v-model="meta_title"
          ...
          counter
          maxlength="70"
          :hint="$t('hint.max70CharactersCanBeUsed')"
        />
      </div>
      <div class="col-xs-12 col-sm-12">
        <div class="text-label">URL handle</div>
        <q-input
          dense
          outlined
          v-model="slug"
      ...
      <div class="col-xs-12 col-sm-12">
        <div class="text-label">Meta description</div>
        <q-input
          dense
          outlined
          ...
          counter
          maxlength="320"
          :hint="$t('hint.max320CharactersCanBeUsed')"
        />
      </div>
      <div v-if="item.meta_keywords" class="col-xs-12 col-sm-12">
        <div class="text-label">Meta keywords</div>
        <q-select
          dense
          outlined
...
export default {
  ...
  methods: {
    onChange(value, key) {
      console.func("components/SeoSection:methods.onChange()", arguments);
      this.item[key] = value;
