      ...
      >
        <template #top-bottom>
          <base-info rounded class="q-mt-md" v-if="hasBlocked">
            {{
              $t("blockedInfo", {
                release: $core.formatDateAsHuman(blocked.release_at),
              })
            }}
          </base-info>
        </template>
...
import PaymentConfirm from "src/components/subscription/PaymentConfirm.vue";
  ...
  methods: {
    ...
    async rowClicked(evt, row) {
      ...
      this.book(row)
        .then(({ payment }) => {
          if (payment) {
            // process payment and book a class
            ...
            this.$q
              .dialog({
                ...
                componentProps: {
                  ...
                  invoice: payment,
                  type: "order",
                  message: `To confirm your booking, please complete payment of ${amount}.`,
